...
export const generateAiJourneyChunks = async (title, timeline, description) => {
    if (!GEMINI_API_KEY) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error("Gemini API request failed with status:", response.status, "Body:", errorBody);
            return generateDefaultChunks();
        }
