import { evaluateAssessment } from "../ai/ai-unlock.js";
import { generateAiJourneyChunks } from "../ai/ai-service.js";
...
const calculateGoalProgress = (goal) => {
    ...
    if (totalChunks === 0)
        return 0;
    const completedChunks = goal.chunks.filter((chunk) => chunk.status === "COMPLETED").length;
...
export const createNewGoal = async (req, res) => {
    ...
    try {
        ...
        const chunksToCreate = journeyChunks.map((chunk, index) => ({
            ...chunk,
...
export const assessChunk = async (req, res) => {
